...
class MetaTrainer(object):
    ...
    def __init__(self, args):
        ...
        # 三级目录
        if args.support_aug :
            save_path1 = 'support_aug_' + str(args.support_aug)
            if args.query_aug :
                save_path1 += '_query_aug_' + str(args.query_aug)
            if args.support_aug2 :
                save_path1 += '_support_aug2_' + str(args.support_aug2)    
        elif args.query_aug :
            save_path1 = 'query_aug_' + str(args.query_aug)
        else:
        ...
        #num_channels, num_points = self.testset.data.shape[1], self.testset.data.shape[2]
        self.model = MtlLearner(self.args, mode='meta', num_cls=2,
    ...
    def train(self):
        ...
        if torch.cuda.is_available():
            ...
            one_hot_val_label_shot = one_hot_val_label_shot.type(torch.cuda.LongTensor)
        else:
            val_label_shot = val_label_shot.type(torch.LongTensor)
            one_hot_val_label_shot = one_hot_val_label_shot.type(torch.LongTensor)
        ...
        for epoch in range(1, self.args.max_epoch + 1):
            ...
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                if self.args.support_aug:
                    if self.args.support_aug == 'ma':
                        #mix-up使用的是one-hot编码
                        new_shot_data, new_shot_label = data_aug(data_shot, one_hot_label_shot, 
                                                        self.args.support_aug)
                    ...
                    merged_shot = data_shot
                    merged_label_shot = label_shot

                ## second data augmentation for support data
                #这里我们设定第一种方法为"ma”,mix-up使用的是one-hot编码
                # 第二种数据增强则在原始样本上操作,然后与merged_shot和merged_label_shot进行拼接
                if self.args.support_aug2:
                    new_shot_data2, new_shot_label2 = data_aug(data_shot, one_hot_label_shot, 
                                                        self.args.support_aug2)

                    merged_label_shot = torch.cat((merged_label_shot, new_shot_label2), dim=0)
                    
                    merged_shot = torch.cat((merged_shot, new_shot_data2), dim=0)         
           

                ## data augmentation for query data
                if self.args.query_aug:
                    if self.args.query_aug == 'ma':
                        new_query_data, new_query_label = data_aug(data_query, one_hot_label, 
                                                    self.args.query_aug)
                        merged_label_query = torch.cat((one_hot_label, new_query_label), dim=0)

                    else:
                        new_query_data, new_query_label = data_aug(data_query, label, 
                                                    self.args.query_aug)
                        merged_label_query = torch.cat((label, new_query_label), dim=0)
                    merged_query = torch.cat((data_query, new_query_data), dim=0)
                else:
                    merged_query = data_query
                    merged_label_query = label

                # Output logits for model
                #logits = self.model((data_shot, label_shot, data_query))
                logits = self.model((merged_shot, merged_label_shot, merged_query))
                # Calculate meta-train loss
                if self.args.query_aug == 'ma':
                    loss = F.binary_cross_entropy_with_logits(logits, merged_label_query)
                else:
                    loss = F.cross_entropy(logits, merged_label_query)
                # Calculate meta-train accuracy
                acc = count_acc(logits, merged_label_query)
            ...
            if epoch % 10 == 0:
                ...
                for i, batch in enumerate(self.val_loader, 1):
                    ...
                    data_shot, data_query = data[:p], data[p:]
                    logits = self.model((data_shot, val_label_shot, data_query))
    ...
    def eval(self):
        ...
        #print('Test shot Acc {:.4f}'.format(shot_ave_acc.item()))
